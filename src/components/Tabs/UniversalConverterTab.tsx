...
export const UniversalConverterTab: React.FC<Props> = ({ lang, onOpenQr }) => {
  const t = translations[lang];
  const isFa = lang === 'fa';

  const [inputData, setInputData] = useState(
    `vless://351c9981-04b6-4103-aa4b-864aa9c91469@104.16.1.1:443?type=ws&security=tls&path=/zeus&host=example.workers.dev#⚡-Node-1
vless://351c9981-04b6-4103-aa4b-864aa9c91469@104.17.2.2:443?type=ws&security=tls&path=/zeus&host=example.workers.dev#⚡-Node-2`
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-3xl p-6 border border-white/10 shadow-xl space-y-4 text-xs flex flex-col">
          <div className="flex items-center justify-between">
            <span className="font-bold text-slate-300">کانفیگ‌های ورودی (Single or Multi-line / Sub):</span>
            <button
          ...
          <textarea
            ...
            placeholder="کانفیگ‌های خود را خط به خط یا لینک سابسکریپشن را وارد کنید..."
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar text-xs font-bold">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl cursor-pointer ${
                  targetFormat === 'singbox' ? 'bg-purple-500 text-white' : 'text-slate-400'
                }`}
              >
                Sing-Box JSON
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl cursor-pointer ${
                  targetFormat === 'clash' ? 'bg-blue-500 text-white' : 'text-slate-400'
                }`}
              >
                Clash Meta YAML
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl cursor-pointer ${
                  targetFormat === 'vless' ? 'bg-lime text-black' : 'text-slate-400'
                }`}
              >
                VLESS Links
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl cursor-pointer ${
                  targetFormat === 'base64' ? 'bg-cyan text-black' : 'text-slate-400'
                }`}
              >
                Base64 Sub
              </button>
